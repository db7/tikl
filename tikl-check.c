...
static void vecdir_push(vecdir *vd, directive dir)
{
    if(vd->n == vd->cap) {
...
static void vecdir_free(vecdir *vd)
{
    for(size_t i = 0; i < vd->n; i++) {
        directive *d = &vd->v[i];
        if(d->has_regex) regfree(&d->regex);
        free(d->pattern_text);
    }
    free(vd->v);
}
...
{
    while(*s) strbuf_append_char(sb, *s++);
}

static char *strbuf_steal(strbuf *sb)
{
    if(!sb->data) {
        sb->data = xstrdup("");
        sb->len = 0;
...
static void add_prefix(vecstr *prefixes, const char *name)
{
    if(!name || *name == '\0') die("tikl-check: empty --check-prefix value");
...
static subst_table load_substs(void)
{
    subst_table table = {0};
    const char *env = getenv("TIKL_CHECK_SUBSTS");
    if(!env || !*env) return table;
    const char *p = env;
    while(*p) {
...
static void free_substs(subst_table *table)
{
...
static const char *lookup_subst(const subst_table *table, const char *key, size_t len)
{
    for(size_t i = 0; i < table->n; i++) {
...
static void append_literal_segment(strbuf *sb, const char *seg, size_t len, bool escape_literals)
{
    if(!escape_literals) {
        for(size_t i = 0; i < len; i++) strbuf_append_char(sb, seg[i]);
        return;
    }
    const char *meta = "][][.^$\\*/+?{}()|";
    for(size_t i = 0; i < len;) {
        ...
        if(c == '\\' && i + 1 < len) {
            char next = seg[i + 1];
            if(strchr(meta, next)) strbuf_append_char(sb, '\\');
            strbuf_append_char(sb, next);
            i += 2;
            continue;
        }
        if(strchr(meta, c)) strbuf_append_char(sb, '\\');
        strbuf_append_char(sb, c);
        i++;
    }
}

static const char *find_block_close(const char *start)
{
    const char *p = start;
...
static char *build_regex_from_pattern(const char *pattern, bool lit_compat)
{
    strbuf sb = {0};
    ...
    while(*p) {
        ...
        for(const char *q = inner; q < close; ++q) strbuf_append_char(&sb, *q);
        p = close + 2;
    }
    return strbuf_steal(&sb);
}

static const char *match_directive(const char *line, const char *prefix, const char *suffix)
{
    char needle[256];
    size_t need = strlen(prefix) + strlen(suffix) + 1;
    if(need > sizeof(needle)) die("tikl-check: prefix too long");
...
static void add_directive(vecdir *dirs, check_kind kind, prefix_state *state,
...
static void parse_test_file(const char *path, const vecstr *prefixes,
                            prefix_state *states, bool lit_compat,
                            const subst_table *subs, vecdir *dirs, int *status)
{
    FILE *f = fopen(path, "r");
    if(!f) die("tikl-check: cannot open %s: %s", path, strerror(errno));
    char *line = NULL;
    size_t cap = 0;
    while(true) {
        ...
        for(size_t i = 0; i < prefixes->n && !matched; i++) {
            ...
            if((rest = match_directive(line, prefixes->v[i], "-NEXT:"))) {
                ...
                while(*digits && isspace((unsigned char)*digits)) digits++;
...
static void read_output(vecstr *lines)
{
    char *line = NULL;
    size_t cap = 0;
    ssize_t n;
...
static bool regex_matches(const regex_t *re, const char *line)
{
    return regexec(re, line, 0, NULL, 0) == 0;
}

static void handle_check_forward(const directive *dir, const vecstr *lines, int *status)
{
...
static void handle_check_next(const directive *dir, const vecstr *lines, int *status)
{
...
static void handle_check_same(const directive *dir, const vecstr *lines, int *status)
{
    if(!dir->prefix->have_last) {
...
static void handle_check_empty(const directive *dir, const vecstr *lines, int *status)
{
...
static void handle_check_not(const directive *dir, const vecstr *lines, int *status)
{
    for(size_t i = 0; i < lines->n; i++) {
        if(regex_matches(&dir->regex, lines->v[i])) {
...
static void handle_check_count(const directive *dir, const vecstr *lines, int *status)
{
    unsigned found = 0;
    for(size_t i = 0; i < lines->n; i++) {
        if(regex_matches(&dir->regex, lines->v[i])) found++;
    }
...
static void run_directives(const vecdir *dirs, const vecstr *lines, int *status)
{
