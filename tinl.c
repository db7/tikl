...
static void copy_str(char *dst, size_t cap, const char *src, const char *what)
{
	size_t needed = strlen(src) + 1;
	if(needed > cap) die("%s too long", what);
	memcpy(dst, src, needed);
}

static void join_with_space(char *dst, size_t cap, const char *lhs,
...
static bool has_feature(vecstr *features, const char *name)
{
	for(size_t i = 0; i < features->n;
	        i++) if(strcmp(features->v[i], name) == 0) return true;
	return false;
}
...
static bool build_temp_path(char *buf, size_t cap, const char *root,
                            const char *leaf)
{
